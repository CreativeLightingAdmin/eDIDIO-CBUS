...
local function encode_repeated_uint32(field_number, values)
    ...
    for _, value in ipairs(values) do
        encoded_values = encoded_values .. encode_uint32(field_number, value)
    end
...
local function encode_dali_message(dali_message)
    ...
    -- Encode the action oneof field (only one of these will be set)
    ...
    -- Encode the params oneof field (only one of these will be set)
    ...
    elseif dali_message.params.dtr then
        -- Assuming DTRPayloadMessage is just another uint32 for simplicity
        encoded_message = encoded_message .. encode_uint32(10, dali_message.params.dtr)
    end
...
local function decode_dali_message(data)
    ...
    while pos <= #data do
        ...
        elseif field_number == 10 and wire_type == 2 then -- dtr (DTRPayloadMessage)
            dali_message.params = dali_message.params or {}
            dali_message.params.dtr, pos = decode_length_delimited(data, pos) -- Assume dtr is length-delimited
